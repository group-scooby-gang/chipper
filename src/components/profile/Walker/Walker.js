...
import {getWalkerProfile, getWalkerHistory} from "./../../../redux/profileReducer"
import "./walker.css"
        ...
        const mappedProfile = this.props.walkerProfle.map((val, i) => {
            return (
                <div className="walkerProfileCard">
                    <h1>{val.firstname} {val.lastname}'s <span>profile</span></h1>
                    <p>{val.experience}</p>
                    <p>Youn are a {val.category}</p>
                    ...
                    <header>
                    <h6>{val.address}, {val.city} {val.zip}, {val.state}</h6>
                    </header>
                </div>
            )
        })
...
export default connect(mapStateToProps, {getWalkerProfile, getWalkerHistory})(Walker);